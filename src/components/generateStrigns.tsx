...
const cadena = (match: Matches) => {
  ...
  let local = equipos.some((e: { equipo: string | undefined; }) => e.equipo == home?.Name);
  // me fijo si el equipo visitante esta elegido
  let visita = equipos.some((e: { equipo: string | undefined; }) => e.equipo == away?.Name);
  // se queda con el *nomEquipo*(persona) local en el caso que este elegido, si no solo nomEquipo
  let pers1 = equipos.filter((e: { equipo: string | undefined; }) => e.equipo == home?.Name)[0]
    ? '*' +
      equipos.filter((e: { equipo: string | undefined; }) => e.equipo == home?.Name)[0]['equipo'] +
      '* ' +
      '(' +
      equipos.filter((e: { equipo: string | undefined; }) => e.equipo == home?.Name)[0]['persona'] +
      ')'
    : home?.Name;
  // se queda con el *nomEquipo*(persona) visitante en el caso que este elegido, si no solo nomEquipo
  let pers2 = equipos.filter((e: { equipo: string | undefined; }) => e.equipo == away?.Name)[0]
    ? '*' +
      equipos.filter((e: { equipo: string | undefined; }) => e.equipo == away?.Name)[0]['equipo'] +
      '* ' +
      '(' +
      equipos.filter((e: { equipo: string | undefined; }) => e.equipo == away?.Name)[0]['persona'] +
      ')'
    : away?.Name;
